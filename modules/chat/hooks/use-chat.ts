...
import { createGeneration, getGenerations, saveGeneratedCode } from "../server/create-code";
...
export function useUpdateGeneration(){
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn:({id,code}: {id: string; code: string})=>saveGeneratedCode(id,code),
        onSuccess:(_,variables) => {
            queryClient.invalidateQueries({ queryKey: zyroKeys.all });
            queryClient.invalidateQueries({ queryKey: zyroKeys.detail(variables.id) });
        }
    })
}
